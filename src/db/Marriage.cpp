...
namespace marriage
{
	...
	void CManager::Add(DWORD dwPID1, DWORD dwPID2, const char* szName1, const char* szName2)
	{
		...
		p.dwPID2 = dwPID2;
		p.tMarryTime = now;
		std::strncpy(p.szName1, szName1, sizeof(p.szName1));
		std::strncpy(p.szName2, szName2, sizeof(p.szName2));
	...
	void CManager::OnSetup(CPeer* peer)
	{
		...
		for (itertype(m_Marriages) it = m_Marriages.begin(); it != m_Marriages.end(); ++it)
		{
...

			{
				...
				std::strncpy(p.szName1, pMarriage->name1.c_str(), sizeof(p.szName1));
				std::strncpy(p.szName2, pMarriage->name2.c_str(), sizeof(p.szName2));
