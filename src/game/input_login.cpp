...
void CInputLogin::Login(LPDESC d, const char * data)
{
	...
	if (g_iUseLocale && !test_server)
	{
		...
		std::strncpy(failurePacket.szStatus, "VERSION", sizeof(failurePacket.szStatus));
	...
	{
		failurePacket.header = HEADER_GC_LOGIN_FAILURE;
		std::strncpy(failurePacket.szStatus, "SHUTDOWN", sizeof(failurePacket.szStatus));
		d->Packet(&failurePacket, sizeof(TPacketGCLoginFailure));
		return;
	}
	...
	if (g_iUserLimit > 0)
	{
		...
		{
			failurePacket.header = HEADER_GC_LOGIN_FAILURE;
			std::strncpy(failurePacket.szStatus, "FULL", sizeof(failurePacket.szStatus));
			d->Packet(&failurePacket, sizeof(TPacketGCLoginFailure));
			return;
		}
	}
	...
	std::strncpy(login_packet.login, login, sizeof(login_packet.login));
	std::strncpy(login_packet.passwd, pinfo->passwd, sizeof(login_packet.passwd));
...
void CInputLogin::LoginByKey(LPDESC d, const char * data)
{
	...
	std::strncpy(ptod.szLogin, login, sizeof(ptod.szLogin));
	ptod.dwLoginKey = pinfo->dwLoginKey;
	thecore_memcpy(ptod.adwClientKey, pinfo->adwClientKey, sizeof(DWORD) * 4);
	std::strncpy(ptod.szIP, d->GetHostName(), sizeof(ptod.szIP));
...
void CInputLogin::ChangeName(LPDESC d, const char * data)
{
	...
	std::strncpy(pdb.name, p->name, sizeof(pdb.name));
		...
		BYTE bDex)
{
	...
	std::strncpy(table->name, name, sizeof(table->name));
...
void CInputLogin::CharacterDelete(LPDESC d, const char * data)
{
	...
	std::strncpy(player_delete_packet.private_code, pinfo->private_code, sizeof(player_delete_packet.private_code));
